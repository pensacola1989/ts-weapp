...
interface WXNetAPIUploadFileObj {
	url: string;
	filePath: string;
	name: string;
	header?: Object;
...
type WxResponse = { errMsg: string, data: any, statusCode: number };
interface WXNetAPI {
	request(obj: WXNetAPIRequestObj): Promise<WxResponse>;
...
interface TapEvent {
	currentTarget: TagEventTarget;
	detail: any;
	target: TagEventTarget;
	timeStamp: number;
	type: string;
}
...
interface WXMediaAPI {
	...
	chooseImage(obj: WXChooseImageObj): Promise<{ tempFilePaths: string[], [propName: string]: any }>
	// chooseImage(obj: WXChooseImageObj);
...
interface WXLocationAPI {
	getLocation(type?: string): Promise<{ latitude: number, longitude: number, speed: number, accuracy: number }>;
	chooseLocation(obj: WXGetLocationObj);
	openLocation(obj: WXOpenLocationObj): Promise<any>;
}
...
interface WXDeviceAPI {
	...
	getSystemInfo(): Promise<WXSystemInfo>;
